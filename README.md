...
1. Using `dev_overrides` path
    ...
    Please note that after `dev_overrides` is added, you cannot run `terraform init` if 
    `registry.terraform.io/retailnext/scylladb` appears in the terraform code. That means,
    if you have other non-local providers in your terraform code, you will not be able to
    run `terraform init`. In this case, use the following "release" binary method.
