...
func TestAccProviderConfigmTLSInlineCACert(t *testing.T) {
	caCert, err := testutil.GenerateTestCACert()
	if err != nil {
		t.Fatalf("failed to generate CA certificate: %s", err)
	}
	...
	// Pass CA cert inline via ca_cert attribute rather than via env var.
	providerConfigCAFmt := `
provider "scylladb" {
  host    = "%s"
  ca_cert = <<-EOT
...
`
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(providerConfigCAFmt, host, string(caCertPEM)),
			},
		},
	})
}

func TestAccProviderConfigCACertConflict(t *testing.T) {
	caCert, err := testutil.GenerateTestCACert()
	if err != nil {
		t.Fatalf("failed to generate CA certificate: %s", err)
	}
	...
	f, err := os.CreateTemp(t.TempDir(), "ca_cert_*.pem")
	if err != nil {
		t.Fatalf("failed to create temp CA cert file: %s", err)
	}
	if _, err = f.Write(caCertPEM); err != nil {
		t.Fatalf("failed to write temp CA cert file: %s", err)
	}
	if err = f.Close(); err != nil {
		t.Fatalf("failed to close temp CA cert file: %s", err)
	}
		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`Conflicting CA Certificate Configuration`),
			},
		},
	})
}
